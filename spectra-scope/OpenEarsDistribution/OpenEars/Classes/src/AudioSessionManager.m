...
void performRouteChange() {
    ...
    //CFRelease(audioRoute); // When I release these things get weird, and when I don't release them they cause tiny one-off leaks. I think it's a special circumstance to be using a property return method that creates a string and then doing something async with it. Until there's time for a lot more attention, tiny one-off leak is preferable to weirdness.
}
