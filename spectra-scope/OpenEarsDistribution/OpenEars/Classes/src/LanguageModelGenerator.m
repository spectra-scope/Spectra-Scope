...
- (void) createLanguageModelFromFilename:(NSString *)fileName {
	...
	[cmuCLMTKModel runCMUCLMTKOnCorpusFile:[self.pathToCachesDirectory stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.corpus",fileName]] withDMP:outputDMP];
...
- (void) checkPhoneticDictionaryAtAcousticModelPath:(NSString *)acousticModelPath {
        ...
        if(![[NSFileManager defaultManager] isReadableFileAtPath:acousticModelPath]) NSLog(@"Error: the default phonetic dictionary %@ can't be found in the app bundle but the app is attempting to access it, most likely there will be a crash now.",acousticModelPath);
        
}

- (NSDictionary *) assembleUserInfoDictionaryForLanguageModelsForFilename:(NSString *)fileName {
   
    NSArray *objectsArray = [NSArray arrayWithObjects:[NSString stringWithFormat:@"%@.DMP",fileName],[NSString stringWithFormat:@"%@.dic",fileName],[self.pathToCachesDirectory stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.DMP",fileName]],[self.pathToCachesDirectory stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.dic",fileName]],nil];
...
- (NSError *) generateLanguageModelFromTextFile:(NSString *)pathToTextFile withFilesNamed:(NSString *)fileName  forAcousticModelAtPath:(NSString *)acousticModelPath{
    ...
    // Die if we can't read a file at that location at all.
...
- (NSError *) generateLanguageModelFromArray:(NSArray *)languageModelArray withFilesNamed:(NSString *)fileName forAcousticModelAtPath:(NSString *)acousticModelPath {
    ...
    NSArray *dictionaryResultsArray = [self performDictionaryLookup:sortedArray inString:pronunciationDictionary forAcousticModelAtPath:acousticModelPath]; // Do the dictionary pronunciation lookup.
	...
	if (error){ // If this fails, return an error.
	...
	return [NSError errorWithDomain:NSCocoaErrorDomain code:0 userInfo:[self assembleUserInfoDictionaryForLanguageModelsForFilename:fileName]]; // Return the code-0 error with the paths in the userInfo dictionary.
}
