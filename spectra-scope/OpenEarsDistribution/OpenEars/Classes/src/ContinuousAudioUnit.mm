...
OSStatus testFileLoad(const char * testFileName) {
    ...
    if(statusError) {
        NSLog(@"AudioFileCreateWithURL Error: %s", FormatOSStatusError(errorString,statusError)); 
        free(errorString);
        return statusError;
    }
    ...
    if(statusError) {
        NSLog(@"AudioFileWriteBytes Error: %s", FormatOSStatusError(errorString,statusError)); 
        free(errorString);
        return statusError;
    }
